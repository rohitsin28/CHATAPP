...
export const createNewChat = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (isExistingChat) return res.status(200).json({ message: 'Chat already exists', chat: isExistingChat });
    ...
    const chatWithUserData = await Promise.all(
      chats.map(async (chat) => {
        ...
        try {
          ...
          console.log("Failed to fetch user data for chat participant:", otherUserId);
...
export const sendMessage = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!senderId) return res.status(401).json({ message: 'You are not Authorized!' });
...
export const getMessagesByChat = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!userId) return res.status(401).json({ message: 'You are not Authorized!' });
    if (!chatId) return res.status(400).json({ message: 'Chat Id is required!' });
    ...
    if (!otherUserId) return res.status(400).json({ message: 'No other user found in chat!' });
    ...
    try {
      ...
      // Socket notification for read receipts
      ...
      return res.json({ message: 'Messages fetched successfully', messages, user: userData, markedAsSeen: messagesToMarkSeen.length });
    } catch (error) {
