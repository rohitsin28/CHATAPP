...
const ChatHeader = ({
  ...
  const isOnleUser = user && onlineUsers.includes(user._id);
  return (
    <>
      {/* Mobile menu toggle button */}
      <div className="sm:hidden fixed top-4 right-4 z-30">
        <button
          className="p-3 bg-gray-800 rounded-l-lg hover:bg-gray-700 transition-colors"
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${
                          isOnleUser ? "bg-green-500" : "bg-gray-500"
                        }`}
                      ></div>
                      <span className={`text-sm font-medium ${isOnleUser ? 'text-green-500' : 'text-gray-400'}`}>{isOnleUser ? 'Online' : 'Offline'}</span>
                    </div>
                  )}
                </div>
              </div>
